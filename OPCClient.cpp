// OPC Client for integration service
//

// This is a modified version of the OPC Client made available by the professor
// Luiz T. S. Mendes, which itself is a modified version of the
// "Simple OPC Client" originally developed by Philippe Gras(CERN) for 
// demonstrating the basic techniques involved in the development of an 
// OPC DA client 

// The modifications are the change of the COM threading model to MTA, the
// addition of multiple items and groups, the implementation of a WriteItem
// function, which writes synchronously to the OPC server and compatibilization
// of the main function with the Windows threads API.
//
...
DWORD WINAPI OpcClient(LPVOID dataForThreads)
{
	...
	while (1) {
		...
		if (data->writeReqState == WRITE_REQUESTED) {
			...
			printf("Indicator state: %s\n", params->indicatorState ? "TRUE" : "FALSE");
